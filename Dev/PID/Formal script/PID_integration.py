import sensor, image, time,math,serial
...
#6）2轴3轴转化/PID部分
    ...
    舵机a=list[0,0.577*实际距离参数] 
    舵机b=list[-0.289*实际距离参数,-0.5*实际距离参数]
    舵机c=list[0.289*实际距离参数,-0.5*实际距离参数]
...
#7）最终输出
    ...
    自动平衡舵机偏转值=list[PID偏转角度a,PID偏转角度b,PID偏转角度c] 
    Leapmotion控制偏转值=list[LEAM偏转角度a,LEAM偏转角度b,LEAM偏转角度c] 
    最终输出=list[0,0,0] 
    
    Leapmotion接入=1 # 待设置判断leapmotion是否接入
    
    if Leapmotion接入==1:    #点触模式下，绘制两个按键的颜色框
        Leapmotion控制偏转值=最终输出
    else:
        自动平衡舵机偏转值=最终输出
