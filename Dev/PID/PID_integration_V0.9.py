...
import cv2
import numpy as np
import time,math,serial,Leap,threading
from pythonosc import udp_client
from leap_motion_listener import SampleListener


# 创建Leap Motion监听器的全局实例
...
# 二.视觉识别部分
    ...
    # 在这个函数中使用Leap Motion数据
        hand1_ty, hand0_ty = listener.get_hand_positions()
        #print("From some_other_function - Hand 1 Y:", hand1_ty, "Hand 0 Y:", hand0_ty)
    ...
    def main():
        ...
        try:
            while True:
            # 主循环也可以使用Leap Motion数据
                hand1_ty, hand0_ty = listener.get_hand_positions()
                #print("From main loop - Hand 1 Y:", hand1_ty, "Hand 0 Y:", hand0_ty)
                some_other_function()
                time.sleep(0.1)
    ...
    x轴控制=int(hand1_ty)    #防报错预设
    y轴控制=int(hand0_ty)     #防报错预设
    ...
    LEAP有效输入值=[x轴控制,y轴控制]
    LEAP虚拟小球位置=[LEAP有效输入值[0]*LEAP变换x,LEAP有效输入值[1]*LEAP变换y]
    LEAP接入=0
    
    x轴控制=int(x轴控制)
    y轴控制=int(y轴控制)
    
    if x轴控制 != 125 & y轴控制 != 125:
        LEAP接入=1
...
# 四.PID控制部分
    for n in range(0,2):
        ...
        if LEAP接入==1:
            映射坐标 = [小球位置映射系数 *LEAP虚拟小球位置[0], 小球位置映射系数 * LEAP虚拟小球位置[1]]
    ...
    print("From main loop - Hand 1 Y:", hand1_ty, "Hand 0 Y:", hand0_ty)
    print(f"{LEAP有效输入值}{LEAP虚拟小球位置}(执行量:{执行量})(舵机距离:{球舵距离})(最终输出:{最终输出})")
    ...
    #cv2.circle(frame, (LEAP虚拟小球位置[0], LEAP虚拟小球位置[1]), 10, (0, 0, 255), -1) # LEAP虚拟小球位置
