...
Encoder enc_a(A_enc_a, B_enc_a);
Encoder enc_b(A_enc_b, B_enc_b);
Encoder enc_c(A_enc_c, B_enc_c);
//-------------------------------------------------------
//stepper motor sruff
//-------------------------------------------------------
#define motor_a 1
...
void loop()
{
  ...
  if (compute == 1)
  {
    ...
    stepper_run(motor_a, set_speed_a);
    stepper_run(motor_b, set_speed_b);
    stepper_run(motor_c, set_speed_c);
...
void calc_PID()
{
  measured_angle_a = enc_a.read();
  measured_angle_b = enc_b.read();
  measured_angle_c = enc_c.read();
...
void stepper_setup()
{
  ...
  digitalWrite(en_a_pin, HIGH); // disable motor
  digitalWrite(en_b_pin, HIGH); // disable motor
  digitalWrite(en_c_pin, HIGH); // disable motor
  ...
  TCCR5B = _BV(CS50) | _BV(WGM53) | _BV(WGM52);  //internal clock with CLK/1 prescaling
...
void motor_power(bool power)
{
  if (power == 1)
  {
    digitalWrite(en_a_pin, LOW); // enable motor
    digitalWrite(en_b_pin, LOW); // enable motor
    digitalWrite(en_c_pin, LOW); // enable motor
  }
  else
  {
    digitalWrite(en_a_pin, HIGH); // disable motor
